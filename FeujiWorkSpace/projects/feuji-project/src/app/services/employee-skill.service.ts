...
export class EmployeeSkillService 
{
  getSkillCategoryIdByName(skillCategory: string): Observable<any>  {
      return this.http.get<any>(`http://13.48.82.196:8089/api/referencetype/getref/${skillCategory}`)
  }
  
  
  getSkillCategoryTypeId(selectedsubskillcategory: string): Observable<any> 
  {
    return this.http.get<any>(`http://13.48.82.196:8089/api/referencetype/getreference/${selectedsubskillcategory}`)
  }
  ...
  {
    return this.http.get<any[]>(`http://13.48.82.196:8089/api/referencedetails/getreference/Skill Category`)
  }
...
saveSubSkillCategoryAdmin(newSubCategory: SubSkillCategoryBean): Observable<SubSkillCategoryBean> {
  const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
  return this.http.post<SubSkillCategoryBean>('http://13.48.82.196:8089/api/referencedetails/addSubSkill', newSubCategory, { headers });
}
...
public updateStatusAdmin(skillIds: any, status: any): Observable<any> {
    return this.http.put<any>(`http://13.48.82.196:8087/api/skill/updateStatus?skillIds=${skillIds}&status=${status}`,{});
